...
public class OddJobTrigger {
    ...
    public static void trigger(int jobId,
                               ...
                               int failRetryCount,
                               String executorShardingParam, // 默认null
        ...
        if (ExecutorRouteStrategyEnum.SHARDING_BROADCAST==ExecutorRouteStrategyEnum.match(jobInfo.getExecutorRouteStrategy(), null)
                ...
                && shardingParam==null) {
            for (int i = 0; i < group.getRegistryList().size(); i++) { // 分片执行，集群中所有分区都得执行
                processTrigger(group, jobInfo, finalFailRetryCount, triggerType, i, group.getRegistryList().size());
            }
        } else {
            if (shardingParam == null) {
                shardingParam = new int[]{0, 1}; //非分片广播路由方式，这两个参数无用
            }
     ...
     */
    private static void processTrigger(OddJobGroup group, OddJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total){

        // param
        ExecutorBlockStrategyEnum blockStrategy = ExecutorBlockStrategyEnum.match(jobInfo.getExecutorBlockStrategy(), ExecutorBlockStrategyEnum.SERIAL_EXECUTION);  // block strategy
        ExecutorRouteStrategyEnum executorRouteStrategyEnum = ExecutorRouteStrategyEnum.match(jobInfo.getExecutorRouteStrategy(), null);    // route strategy
        String shardingParam = (ExecutorRouteStrategyEnum.SHARDING_BROADCAST==executorRouteStrategyEnum)?String.valueOf(index).concat("/").concat(String.valueOf(total)):null; // i/len
        ...
        OddJobAdminConfig.getAdminConfig().getOddJobLogDao().save(jobLog);
        logger.debug(">>>>>>>>>>> odd-job trigger start, jobId:{}", jobLog.getId());
        ...
        OddJobAdminConfig.getAdminConfig().getOddJobLogDao().updateTriggerInfo(jobLog);

        logger.debug(">>>>>>>>>>> odd-job trigger end, jobId:{}", jobLog.getId());
    }
    ...
    public static ReturnT<String> runExecutor(TriggerParam triggerParam, String address){
        ...
        try {
            ...
            runResult = executorBiz.run(triggerParam); // 此处返回的只是调度结果
        } catch (Exception e) {
            logger.error(">>>>>>>>>>> odd-job trigger error, please check if the executor[{}] is running.", address, e);
