...
public class JobApiController {
    ...
    public ReturnT<String> api(HttpServletRequest request, @PathVariable("uri") String uri, @RequestBody(required = false) String data) {
        ...
        if (OddJobAdminConfig.getAdminConfig().getAccessToken()!=null
                && OddJobAdminConfig.getAdminConfig().getAccessToken().trim().length()>0
                && !OddJobAdminConfig.getAdminConfig().getAccessToken().equals(request.getHeader(OddJobRemotingUtil.ODD_JOB_ACCESS_TOKEN))) {
