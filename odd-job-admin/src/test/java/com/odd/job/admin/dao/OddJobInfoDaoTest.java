...
public class OddJobInfoDaoTest {
	private static Logger logger = LoggerFactory.getLogger(OddJobInfoDaoTest.class);
	...
	public void pageList(){
		List<OddJobInfo> list = oddJobInfoDao.pageList(0, 20, 0, -1, null, null, null);
		int list_count = oddJobInfoDao.pageListCount(0, 20, 0, -1, null, null, null);

		logger.info("", list);
		logger.info("", list_count);

		List<OddJobInfo> list2 = oddJobInfoDao.getJobsByGroup(1);
	}
	
	@Test
	public void save_load(){
		OddJobInfo info = new OddJobInfo();
		info.setJobGroup(1);
		...
		int count = oddJobInfoDao.save(info);

		OddJobInfo info2 = oddJobInfoDao.loadById(info.getId());
		info.setScheduleType(ScheduleTypeEnum.FIX_RATE.name());
		...
		int item2 = oddJobInfoDao.update(info2);

		oddJobInfoDao.delete(info2.getId());

		List<OddJobInfo> list2 = oddJobInfoDao.getJobsByGroup(1);

		int ret3 = oddJobInfoDao.findAllCount();
